...
**Run predictive risk assessment on planned tasks**

Check if prediction is enabled:

```bash
PREDICTION_ENABLED="false"  # Default disabled
ADAPTIVE_ENABLED=$(cat .planning/config.json 2>/dev/null | grep -o '"enabled": *[^,}]*' | head -1 | grep -o 'true\|false' || echo "false")
PREDICTION_SETTING=$(cat .planning/config.json 2>/dev/null | grep -o '"prediction_enabled": *[^,}]*' | grep -o 'true\|false')

# Prediction requires adaptive to be enabled
if [ "$ADAPTIVE_ENABLED" = "true" ]; then
  if [ -z "$PREDICTION_SETTING" ] || [ "$PREDICTION_SETTING" = "true" ]; then
    PREDICTION_ENABLED="true"
  fi
fi

echo "Prediction enabled: $PREDICTION_ENABLED"
```

**If prediction active AND knowledge base exists:**

1. Source predict-failures.md workflow
2. Run prediction on the draft plan tasks
3. Store RISK_ASSESSMENT result

```bash
if [ "$PREDICTION_ENABLED" = "true" ]; then
  if [ -d ~/.claude/gsd-knowledge ] && [ -f ~/.claude/gsd-knowledge/index.json ]; then
    # Run predictions using predict-failures workflow
    # PLAN_FILE should contain the draft tasks we're about to confirm
    source ~/.claude/get-shit-done/workflows/predict-failures.md 2>/dev/null || true
    run_predictions_safely "$PLAN_FILE" 2>/dev/null || true

    if [ -n "$RISK_ASSESSMENT" ]; then
      echo "$RISK_ASSESSMENT"
      HAS_HIGH_RISK="false"
      if echo "$RISK_ASSESSMENT" | grep -q "HIGH risk"; then
        HAS_HIGH_RISK="true"
      fi
    fi
  fi
...
**Display risk assessment before confirmation:**
- Show after task breakdown
- Before asking "Does this look right?"
- If HIGH risk tasks exist, explicitly note them in confirmation prompt

**Configuration:**
Prediction uses `adaptive.prediction_enabled` (defaults to true when `adaptive.enabled` is true).

```json
{
  "adaptive": {
    "enabled": false,
    "prediction_enabled": true  // Run failure prediction during planning (requires enabled=true)
  }
}
```

**Graceful degradation:**
- If knowledge base doesn't exist: Skip prediction silently
- If prediction fails: Log warning, continue without risk assessment
- Never block planning due to prediction errors
</step>
...
[If RISK_ASSESSMENT is non-empty, display it here]

Does this look right? (yes / adjust / start over)
```

**If HIGH risk detected (HAS_HIGH_RISK="true"):**

Change confirmation prompt to offer alternatives:
...
[RISK_ASSESSMENT with HIGH risk warnings and suggested alternatives]

⚠️ HIGH RISK DETECTED - some tasks have high failure probability.

How to proceed?
- **yes** - Proceed with current plan (be prepared for suggested alternatives if failures occur)
- **adjust** - Revise task breakdown to reduce risk
- **view alternatives** - Re-display suggested alternatives for each HIGH risk task
- **start over** - Return to phase context
```

**Handling "view alternatives":**
If user selects "view alternatives", re-display the alternatives section from RISK_ASSESSMENT and return to the confirmation prompt.

For multiple plans, show each plan with its tasks.

Wait for confirmation:
- If "yes": Proceed to write_phase_prompt
- If "adjust": Revise tasks, re-run prediction
- If "view alternatives": Re-display alternatives, return to confirmation
- If "start over": Return to gather_phase_context
</if>
</step>
